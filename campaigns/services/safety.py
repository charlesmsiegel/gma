...
class CampaignSafetyService:
    ...
    def update_campaign_safety_settings(
        ...
        if not campaign.has_role(updated_by, 'OWNER', 'GM'):
            raise ValidationError("Only campaign owners and GMs can update safety settings")
        ...
        if content_warnings is not None:
            ...
            cleaned_warnings = [
                warning.strip() for warning in content_warnings 
    ...
    def create_safety_agreement(
        ...
        cleaned_warnings = [
            warning.strip() for warning in acknowledged_warnings 
            if warning and warning.strip()
        ]
    ...
    def get_campaign_safety_overview(
        ...
        if campaign.is_member(user):
            ...
            if agreement:
                ...
                unacknowledged = set(campaign.content_warnings) - set(agreement.acknowledged_warnings)
    ...
    def check_user_safety_agreement(
        ...
        if agreement:
            ...
            missing_warnings = set(campaign.content_warnings) - set(agreement.acknowledged_warnings)
    ...
    def get_campaign_agreements_summary(
        ...
        if not campaign.has_role(requesting_user, 'OWNER', 'GM'):
            raise ValidationError("Only campaign owners and GMs can view agreements summary")
    ...
    def bulk_create_agreements(
        ...
        if not campaign.has_role(requesting_user, 'OWNER', 'GM'):
            raise ValidationError("Only campaign owners and GMs can bulk create agreements")
    ...
    def delete_safety_agreement(
        ...
        if not (user == requesting_user or campaign.has_role(requesting_user, 'OWNER', 'GM')):
    ...
    def _get_gm_safety_info(self, campaign: 'Campaign') -> Dict[str, Any]:
        ...
        participants_info = preferences_service.get_users_with_preferences_in_campaign(campaign)

        # Get agreement completion statistics
        total_agreements = CampaignSafetyAgreement.objects.filter(campaign=campaign).count()
        
        # Get all participants count
        from .campaign_services import MembershipService
        membership_service = MembershipService(campaign)
        total_participants = 1 + membership_service.get_campaign_members().count()  # +1 for owner
        ...
        return {
            ...
            'agreement_completion_rate': (
                total_agreements / total_participants * 100 if total_participants > 0 else 0
            ),
