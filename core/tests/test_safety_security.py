...
class SafetySystemPermissionTest(TestCase):
    ...
    def setUp(self):
        ...
        CampaignMembership.objects.create(
            campaign=self.main_campaign, user=self.gm_user, role="GM"
        )
        CampaignMembership.objects.create(
            campaign=self.main_campaign, user=self.player_user, role="PLAYER"
        )
    ...
    def test_private_safety_preferences_access_control(self):
        ...
        response = self.client.get(
            reverse(
                "api:user_safety_preferences", kwargs={"user_id": self.player_user.id}
            )
        )
    ...
    def test_gm_only_safety_preferences_access_control(self):
        ...
        response = self.client.get(
            reverse(
                "api:user_safety_preferences",
                kwargs={"user_id": self.campaign_owner.id},
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["lines"], ["GM only line content"])
    ...
    def test_cross_campaign_access_prevention(self):
        ...
        CampaignMembership.objects.create(
            campaign=self.other_campaign, user=self.malicious_user, role="GM"
        )
        ...
        response = self.client.get(
            reverse(
                "api:campaign_safety", kwargs={"campaign_id": self.main_campaign.id}
            )
        )
        ...
        response = self.client.get(
            reverse(
                ...
                kwargs={"campaign_id": self.main_campaign.id},
            )
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class SafetySystemSecurityTest(TestCase):
    ...
    def setUp(self):
        ...
        self.attacker = User.objects.create_user(
            username="attacker", email="attacker@example.com", password="TestPass123!"
        )
    ...
    def test_sql_injection_prevention_in_safety_preferences(self):
        ...
        for payload in sql_injection_payloads:
            with self.subTest(payload=payload):
                ...
                response = self.client.put(
                    reverse("api:safety_preferences"), malicious_prefs, format="json"
                )
    ...
    def test_xss_prevention_in_safety_content(self):
        ...
        for payload in xss_payloads:
            with self.subTest(payload=payload):
                ...
                response = self.client.put(
                    reverse("api:safety_preferences"), xss_prefs, format="json"
                )
    ...
    def test_json_injection_prevention(self):
        ...
        for payload in json_injection_payloads:
            with self.subTest(payload=payload):
                ...
                response = self.client.put(
                    reverse("api:safety_preferences"), injection_data, format="json"
                )
    ...
    def test_mass_assignment_prevention(self):
        ...
        response = self.client.put(
            reverse("api:safety_preferences"), mass_assignment_attempt, format="json"
        )
    ...
    def test_rate_limiting_enforcement(self):
        ...
        for i in range(50):  # Attempt many rapid requests
            ...
            response = self.client.put(
                reverse("api:safety_preferences"), prefs_data, format="json"
            )
    ...
    def test_concurrent_modification_protection(self):
        ...
        response1 = self.client.put(
            reverse("api:safety_preferences"), update1, format="json"
        )
        response2 = self.client.put(
            reverse("api:safety_preferences"), update2, format="json"
        )
    ...
    def test_input_validation_boundary_conditions(self):
        ...
        response = self.client.put(
            reverse("api:safety_preferences"), boundary_test_data, format="json"
        )
        ...
        response = self.client.put(
            reverse("api:safety_preferences"), many_items_data, format="json"
        )
