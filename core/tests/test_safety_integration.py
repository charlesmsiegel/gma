...
class SafetySystemIntegrationTest(TestCase):
    ...
    def test_complete_safety_system_workflow(self):
        ...
        agreement1 = {
            ...
            "acknowledged_warnings": ["Violence", "Supernatural horror", "Mental health issues"]
        }
        response = self.client.post(
            reverse("api:campaign_safety_agreement", kwargs={"campaign_id": self.campaign.id}),
            agreement1,
            format='json'
        )
        ...
        response = self.client.post(
            reverse("api:pre_scene_safety_check"),
            scene_check,
            format='json'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertFalse(validation_results['is_safe'])  # Should trigger multiple concerns
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)  # Player2 is not GM
    ...
    def test_safety_system_cross_campaign_isolation(self):
        ...
        self.client.force_authenticate(user=self.player2)
        response = self.client.get(
            reverse("api:campaign_safety_agreements", kwargs={"campaign_id": campaign2.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class SafetySystemRealWorldScenariosTest(TestCase):
    ...
    def test_horror_campaign_safety_setup(self):
        ...
        response = self.client.put(reverse("api:safety_preferences"), bob_prefs, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.put(reverse("api:safety_preferences"), carol_prefs, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.put(reverse("api:safety_preferences"), dave_prefs, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        
        # GM reviews safety overview
        self.client.force_authenticate(user=self.gm)
        response = self.client.get(
            reverse("api:campaign_safety_overview", kwargs={"campaign_id": self.horror_campaign.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_realistic_scene_content_validation(self):
        ...
        bob_result = next((r for r in user_results if r['user_id'] == self.player_bob.id), None)
        ...
        carol_result = next((r for r in user_results if r['user_id'] == self.player_carol.id), None)
    ...
    def test_mid_session_safety_intervention(self):
        ...
        response = self.client.post(
            reverse("api:real_time_content_check"),
            real_time_check,
            format='json'
        )
    ...
    def test_session_zero_safety_discussion(self):
        ...
        preferences_data = [
            (self.player_bob, {"lines": ["Graphic violence"], "privacy_level": "gm_only"}),
            (self.player_carol, {"lines": ["Sexual content"], "privacy_level": "campaign_members"}), 
            (self.player_dave, {"veils": ["Racism"], "privacy_level": "campaign_members"})
        ]
        ...
        for player, prefs in preferences_data:
            ...
            response = self.client.put(reverse("api:safety_preferences"), prefs, format='json')
            self.assertEqual(response.status_code, status.HTTP_200_OK)
        
        # GM gets aggregated safety summary for discussion
        self.client.force_authenticate(user=self.gm)
        response = self.client.get(
            reverse("api:campaign_safety_discussion_guide", kwargs={"campaign_id": self.horror_campaign.id})
        )
