...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if success:
            ...
            self.stdout.write(
                self.style.ERROR("❌ Some health checks failed!")
            )
            exit(1)
    ...
    def test_database(self, log=False):
        ...
        try:
            ...
            if result == (1,):
                ...
                self.stdout.write(f"  🏠 Host: {db_settings['HOST']}:{db_settings['PORT']}")
                ...
                self.stdout.write(
                    self.style.ERROR("  ❌ Database connection: Failed (unexpected result)")
                )
                if log:
                    HealthCheckLog.objects.create(
                        service='database',
