...
class ItemForm(forms.ModelForm):
    ...
    def save(self, created_by=None, modified_by=None, commit=True):
        ...
        # Check if this is a new item before saving
        is_new_item = not self.instance.pk
        original_owner = None

        if not is_new_item:
            # Get original owner before saving changes
            original_owner = Item.objects.get(pk=self.instance.pk).owner

        if commit:
            item.save()

            # Handle ownership transfer (both new items and changes)
            if item.owner is not None:
                # Set transfer timestamp for new items or owner changes
                if is_new_item or original_owner != item.owner:
