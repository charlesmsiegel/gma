...
class CampaignItemsView(CampaignItemsMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Item.objects.filter(
            campaign=campaign
        ).select_related('owner', 'created_by').order_by('name')
...
class ItemCreateView(CampaignItemsMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, 
            f"Item '{self.object.name}' created successfully."
        )
        return redirect(self.get_success_url())
        
    def get_success_url(self):
        """Redirect to item detail after creation."""
        return reverse('items:detail', kwargs={
...
class ItemDetailView(CampaignItemsMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        try:
            return get_object_or_404(
                queryset.filter(campaign=campaign),
                pk=item_id
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            "can_edit": user_role in ["OWNER", "GM"] or (
...
class ItemEditView(CampaignItemsMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f"Item '{self.object.name}' updated successfully."
        )
        return redirect(self.get_success_url())
...
class ItemDeleteView(CampaignItemsMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.success(
                request,
                f"Item '{self.object.name}' deleted successfully."
            )
    ...
    def get_success_url(self):
        ...
        return reverse('items:campaign_items', kwargs={
            'campaign_slug': self.get_campaign().slug
        })
