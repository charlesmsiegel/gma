...
class SafetyPreferencesAPITest(TestCase):
    ...
    def test_update_existing_safety_preferences(self):
        ...
        response = self.client.put(
            self.safety_preferences_url, updated_data, format="json"
        )
    ...
    def test_get_other_user_safety_preferences_private(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.user_safety_preferences_url(self.other_user.id))
    ...
    def test_get_other_user_safety_preferences_campaign_members(self):
        """Test getting other user's campaign_members preferences (same campaign)."""
        from users.models.safety import UserSafetyPreferences
    ...
    def test_gm_access_to_gm_only_preferences(self):
        ...
        response = self.client.get(self.user_safety_preferences_url(self.user.id))
    ...
    def test_safety_preferences_field_validation(self):
        ...
        response = self.client.put(
            self.safety_preferences_url, invalid_data, format="json"
        )
    ...
    def test_safety_preferences_special_characters(self):
        ...
        # HTML content should be sanitized for security
        expected_lines = [
            "Content with 'quotes' and \"double quotes\"",
            "Unicode: émotions, 中文, русский",
            "HTML-like: alert('test')",  # XSS script tags removed
            'JSON-like: {"key": "value"}',
        ]
        self.assertEqual(response.data["lines"], expected_lines)
...
class SafetyPreferencesAPISecurityTest(TestCase):
    ...
    def test_cannot_modify_other_user_preferences(self):
        ...
        self.client.put(self.safety_preferences_url, malicious_data, format="json")
    ...
    def test_sql_injection_prevention(self):
        ...
        for injection_attempt in sql_injection_attempts:
            with self.subTest(injection=injection_attempt):
                ...
                response = self.client.put(
                    self.safety_preferences_url, data, format="json"
                )
                ...
                if response.status_code == status.HTTP_200_OK:
                    # If stored, verify it's stored safely
                    if "<script>" in injection_attempt or "<" in injection_attempt:
                        # XSS payloads should be sanitized (HTML tags removed)
                        import bleach

                        expected = bleach.clean(injection_attempt, tags=[], strip=True)
                        self.assertEqual(response.data["lines"][0], expected)
                    else:
    ...
    def test_content_type_validation(self):
        ...
        response = self.client.put(
            self.safety_preferences_url, data, content_type="text/plain"
        )
