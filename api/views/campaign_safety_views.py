        ...
        if serializer.is_valid():
            try:
                updated_campaign = service.update_campaign_safety_settings(
                    ...
                    safety_tools_enabled=serializer.validated_data.get(
                        "safety_tools_enabled"
                    ),
                )

                response_serializer = CampaignSafetySerializer(updated_campaign)
                logger.info(
                    f"Campaign safety settings updated by {request.user.username} "
...
def campaign_safety_agreement_view(request, campaign_id):
    ...
    if request.method == "GET":
        ...
        if serializer.is_valid():
            try:
                agreement = service.create_safety_agreement(
                    campaign=campaign,
                    user=request.user,
                    acknowledged_warnings=serializer.validated_data[
                        "acknowledged_warnings"
                    ],
                    agreed_to_terms=serializer.validated_data.get(
        ...
        try:
            deleted = service.delete_safety_agreement(
                campaign, request.user, request.user
            )
            if deleted:
...
def campaign_safety_agreements_view(request, campaign_id):
    ...
    try:
        agreements_summary = service.get_campaign_agreements_summary(
            campaign, request.user
        )
        return Response(
            {"agreements": agreements_summary["participants"]},
...
def campaign_safety_overview_view(request, campaign_id):
    ...
    try:
        if user_role in ["OWNER", "GM"]:
            # Full overview for GMs/Owners - using only campaign service for now
            campaign_service = CampaignSafetyService()
            campaign_overview = campaign_service.get_campaign_safety_overview(
                campaign, request.user
            )

            # Use campaign service data as the main overview
            combined_overview = {
                "safety_summary": {
            ...
            if campaign_overview.get("gm_info"):
                ...
                # Add participants at top level for test compatibility
                if "participants_info" in campaign_overview["gm_info"]:
                    combined_overview["participants"] = campaign_overview["gm_info"][
                        "participants_info"
                    ]
            ...
            campaign_service = CampaignSafetyService()
            overview = campaign_service.get_campaign_safety_overview(
                campaign, request.user
            )
            ...
            limited_overview = {
                "safety_summary": {
                    ...
                    "warnings_to_acknowledge": overview.get(
...
def campaign_safety_check_view(request, campaign_id):
    ...
    if serializer.is_valid():
        ...
        try:
            compatibility_result = validation_service.check_campaign_compatibility(
                target_user, campaign
            )
