...
class CharacterViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self) -> QuerySet[Character]:
        ...
        # Start with base queryset - use all_objects for soft-deleted chars if requested
        ...
        if campaign_id:
            try:
                ...
                if user_role == "PLAYER" and not user.is_superuser:
                    ...
                    if user_filter:
                        # If user filter is specified, check if they can see that user's chars
            ...
            if user.is_superuser:
                ...
                player_only_campaigns = [
                    camp_id for camp_id, role in user_roles_dict.items() 
                    if role == "PLAYER"
                ]
                
                if player_only_campaigns:
                    # For campaigns where user is only a player, show only their characters
                    # For other campaigns, show all characters
                    player_chars_filter = Q(
                        campaign__id__in=player_only_campaigns,
                        player_owner=user
                    )
                    other_campaigns_filter = Q(
                        campaign__id__in=[
                            camp_id for camp_id in user_roles_dict.keys()
                            if camp_id not in player_only_campaigns
                        ]
                    )
                    queryset = queryset.filter(player_chars_filter | other_campaigns_filter)
        ...
        if campaign_id:
            try:
                ...
                if user_role is None:
                    ...
                    raise PermissionDenied("You must be a member of this campaign to create characters.")
    ...
    def check_object_permissions(self, request, obj):
        ...
        # For delete operations, check delete permissions  
            ...
            if not obj.can_be_deleted_by(request.user):
                from rest_framework.exceptions import PermissionDenied
                raise PermissionDenied("You don't have permission to delete this character.")
